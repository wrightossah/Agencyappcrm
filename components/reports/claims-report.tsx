...
export function ClaimsReport({ timeFrame, dateRange, metrics = [], charts = {}, tables = {} }: ClaimsReportProps) {
  return (
      ...
      <div className="grid gap-6 md:grid-cols-2">
        {Object.entries(charts).map(([title, chartData], index) => (
          <ChartComponent
            ...
            chartData={chartData}
            type={
              (title?.toLowerCase() || "").includes("status") || (title?.toLowerCase() || "").includes("policy type")
                ? "pie"
                : "line"
            }
          />
